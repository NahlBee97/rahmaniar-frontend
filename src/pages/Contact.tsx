const EmailIcon = () => (
  <svg
    className="w-6 h-6 text-blue-400"
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    ></path>
  </svg>
);

const LocationIcon = () => (
  <svg
    className="w-6 h-6 text-blue-400"
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    ></path>
    <path
      d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth="2"
    ></path>
  </svg>
);

const LinkedInIcon = () => (
  <svg
    aria-hidden="true"
    className="w-7 h-7"
    fill="currentColor"
    viewBox="0 0 24 24"
  >
    <path
      clipRule="evenodd"
      d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"
      fillRule="evenodd"
    ></path>
  </svg>
);

const GitHubIcon = () => (
  <svg
    aria-hidden="true"
    className="w-7 h-7"
    fill="currentColor"
    viewBox="0 0 24 24"
  >
    <path
      clipRule="evenodd"
      d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.165 6.839 9.489.5.092.682-.217.682-.482 0-.237-.009-.868-.014-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.031-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.203 2.398.1 2.651.64.7 1.03 1.595 1.03 2.688 0 3.848-2.338 4.695-4.566 4.942.359.308.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.001 10.001 0 0022 12c0-5.523-4.477-10-10-10z"
      fillRule="evenodd"
    ></path>
  </svg>
);

// --- Sub-components ---

const ContactForm = () => (
  <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-8 backdrop-blur-sm">
    <form action="#" className="space-y-6" method="POST">
      <div>
        <label
          className="block text-sm font-medium text-gray-300 mb-2"
          htmlFor="name"
        >
          Full Name
        </label>
        <input
          autoComplete="name"
          className="block w-full rounded-md border border-gray-600 bg-gray-700/50 text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition px-4 py-2 outline-none"
          id="name"
          name="name"
          type="text"
        />
      </div>
      <div>
        <label
          className="block text-sm font-medium text-gray-300 mb-2"
          htmlFor="email"
        >
          Email Address
        </label>
        <input
          autoComplete="email"
          className="block w-full rounded-md border border-gray-600 bg-gray-700/50 text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition px-4 py-2 outline-none"
          id="email"
          name="email"
          type="email"
        />
      </div>
      <div>
        <label
          className="block text-sm font-medium text-gray-300 mb-2"
          htmlFor="message"
        >
          Message
        </label>
        <textarea
          className="block w-full rounded-md border border-gray-600 bg-gray-700/50 text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm transition px-4 py-2 outline-none"
          id="message"
          name="message"
          rows={4}
        ></textarea>
      </div>
      <div>
        <button
          className="w-full bg-blue-500 text-white font-semibold px-6 py-3 rounded-lg hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-gray-900"
          type="submit"
        >
          Send Message
        </button>
      </div>
    </form>
  </div>
);

const ContactDetails = () => (
  <div className="space-y-6">
    <div className="flex items-start space-x-4">
      <div className="bg-blue-500/10 p-3 rounded-full mt-1">
        <EmailIcon />
      </div>
      <div>
        <h3 className="text-lg font-semibold text-white">Email</h3>
        <a
          className="text-blue-400 hover:text-blue-300 transition-colors"
          href="mailto:contact@rahmaniar.dev"
        >
          contact@rahmaniar.dev
        </a>
      </div>
    </div>
    <div className="flex items-start space-x-4">
      <div className="bg-blue-500/10 p-3 rounded-full mt-1">
        <LocationIcon />
      </div>
      <div>
        <h3 className="text-lg font-semibold text-white">Location</h3>
        <p className="text-gray-400">Jakarta, Indonesia</p>
      </div>
    </div>
    <div className="pt-4">
      <h3 className="text-lg font-semibold text-white mb-3">Find me on</h3>
      <div className="flex space-x-4">
        <a
          className="text-gray-400 hover:text-white transition-colors"
          href="#"
        >
          <span className="sr-only">LinkedIn</span>
          <LinkedInIcon />
        </a>
        <a
          className="text-gray-400 hover:text-white transition-colors"
          href="#"
        >
          <span className="sr-only">GitHub</span>
          <GitHubIcon />
        </a>
      </div>
    </div>
  </div>
);

// --- Main Page Component ---

const Contact = () => {
  return (
    <div className="bg-linear-to-br from-[#0a111a] to-[#12222b] min-h-screen text-gray-300 font-sans">
      <main>
        <section className="py-16 sm:py-24">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            {/* Header */}
            <div className="text-center mb-12">
              <h1 className="text-4xl sm:text-5xl font-bold text-blue-400 mb-4">
                Contact Me
              </h1>
              <p className="text-lg text-gray-400 max-w-2xl mx-auto">
                I'm open to collaborations and new opportunities. Feel free to
                reach out.
              </p>
            </div>

            {/* Grid Content */}
            <div className="mt-12 max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-10">
              <div className="md:col-span-3">
                <ContactForm />
              </div>
              <div className="md:col-span-2">
                <ContactDetails />
              </div>
            </div>
          </div>
        </section>
      </main>
    </div>
  );
};

export default Contact;
